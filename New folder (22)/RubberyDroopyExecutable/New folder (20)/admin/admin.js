...
function initDashboard() {
  ...
  async function renderVideos() {
    ...
    if (videoCountEl) {
      videoCountEl.textContent = `Total Videos: ${videos.length}`;
    }
    ...
    videos.forEach((v, idx) => {
      ...
      div.innerHTML = `
        <img src="${v.image}" alt="thumb" style="max-width: 80px; height: 50px; object-fit: cover;">
        <div>
          ...
          <a href="${v.link || '#'}" class="video-link" target="_blank">${v.link || 'No link'}</a>
        </div>
        <button onclick="watchVideo('${v.link || '#'}')">Watch</button>
  ...
  window.watchVideo = function(link) {
    if (link && link !== '#') {
  ...
  window.changeViews = async function(videoId, delta) {
    try {
      const videos = await getVideos();
      const video = videos.find(v => v.id === videoId);
      if (!video) return;

      const newViews = Math.max(0, (video.views || 0) + delta);
  ...
  window.editVideo = async function(videoId) {
    try {
      const videos = await getVideos();
      const v = videos.find(video => video.id === videoId);
      if (!v) return;
      ...
      const newToken = prompt('Edit unlock token amount:', v.token || 10);
