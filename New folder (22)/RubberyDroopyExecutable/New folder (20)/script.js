...
function initApp() {
    try {
        ...
        // Initialize Ad Networks after a delay to ensure scripts are loaded
        setTimeout(() => {
        ...
        if (tgUser) {
            ...
            // Disable button if limit is already reached on load
...
function processPoints(points, isBonus = false, isReferral = false) {
     ...
     localStorage.setItem(`adRotationCounter_${tgUser.id}`, 0); // Reset rotation counter
...
function watchAd() {
    ...
    // Show ad loading and start 10-second countdown
    showAdNotification("এড লোড হচ্ছে... ১০ সেকেন্ড অপেক্ষা করুন");
    startAdCountdown();

    // 70% chance for Monetag ads, 30% chance for Direct Link ads
    const random = Math.random();
    
    if (random < 0.7) {
        // Show Monetag ad (70% chance)
        showMonetagAd();
    } else {
        // Show Direct Link ad (30% chance)
        showDirectLinkAd();
    }
}
...
function showMonetagAd() {
    if (typeof show_9616405 === 'function') {
        show_9616405().then(() => {
            // Monetag ad shown successfully - tokens will be collected after countdown
            console.log("Monetag ad displayed");
        }).catch(error => {
            console.error("Monetag ad error:", error);
            showAdNotification("Monetag এড লোড করতে সমস্যা হয়েছে।");
        });
    } else {
        console.warn("Monetag ad function not found, using countdown system.");
        // Still use countdown system even if Monetag not available
    }
}

// Direct Link Ad Display
function showDirectLinkAd() {
    const directLinkUrl = "https://www.profitableratecpm.com/f5ac5m0g0y?key=49105c11ff8f839ec9ea52e2966422bf";
    
    // Create ad display container
    const adContainer = document.getElementById('unified-ad-slot');
    if (adContainer) {
        adContainer.innerHTML = `
            <div class="direct-link-ad-display">
                <div class="direct-link-ad-content">
                    <div class="direct-link-ad-header">
                        <h3>💰 Direct Link Ad</h3>
                        <p>Click to visit the advertiser's page</p>
                    </div>
                    <div class="direct-link-ad-body">
                        <div class="direct-link-ad-placeholder">
                            <div class="direct-link-ad-icon">🔗</div>
                            <div class="direct-link-ad-text">Direct Link Advertisement</div>
                            <div class="direct-link-ad-timer" id="direct-link-timer">10</div>
                        </div>
                    </div>
                    <div class="direct-link-ad-footer">
                        <button class="btn-3d btn-secondary" onclick="skipDirectLinkAd()">Skip Ad</button>
                        <button class="btn-3d" onclick="completeDirectLinkAd()">Visit Link</button>
                    </div>
                </div>
            </div>
        `;
    ...
    let timeLeft = 10;
    const timerElement = document.getElementById('direct-link-timer');
    ...
    const countdown = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            completeDirectLinkAd();
        }
    }, 1000);

    // Store timer reference for cleanup
    window.directLinkCountdown = countdown;
}

function skipDirectLinkAd() {
    if (window.directLinkCountdown) {
        clearInterval(window.directLinkCountdown);
    }
    ...
    showAdNotification("Ad skipped. No points earned.");
}

function completeDirectLinkAd() {
    if (window.directLinkCountdown) {
        clearInterval(window.directLinkCountdown);
    }
    ...
    // Open the direct link in new tab
    const directLinkUrl = "https://www.profitableratecpm.com/f5ac5m0g0y?key=49105c11ff8f839ec9ea52e2966422bf";
    window.open(directLinkUrl, '_blank');
    
    // Tokens will be collected automatically by the main countdown system
    showAdNotification("Direct Link এড সম্পন্ন! টোকেন কালেক্ট হচ্ছে...");
}

// Add countdown function for automatic token collection
function startAdCountdown() {
    ...
    if (watchAdBtn) {
        watchAdBtn.disabled = true;
        watchAdBtn.textContent = `এড দেখছেন... ${timeLeft}s`;
    }
    
    const countdown = setInterval(() => {
        timeLeft--;
        if (watchAdBtn && timeLeft > 0) {
            watchAdBtn.textContent = `এড দেখছেন... ${timeLeft}s`;
        }
        
        if (timeLeft <= 0) {
            clearInterval(countdown);
            // Automatically collect tokens after 10 seconds
            handleAdSuccess();
            
            // Reset button
            if (watchAdBtn) {
                watchAdBtn.disabled = false;
                watchAdBtn.textContent = 'Watch Ad (+ 10 টোকেন)';
            }
        }
    }, 1000);
}
...
function initializeAdNetworks() {
    // Monetag ads (70%) + Direct Link ads (30%) system
    console.log('Ad system initialized - Monetag (70%) + Direct Link (30%)');
    
    // Update loading states
    updateAdLoadingStates();
}

// Update Ad Loading States - Removed to clean UI
function updateAdLoadingStates() {
    // Ad loading states removed for cleaner UI
    console.log('Ad loading states removed for better mobile experience');
}
...
window.initializeAdNetworks = initializeAdNetworks;
window.showMonetagAd = showMonetagAd;
window.showDirectLinkAd = showDirectLinkAd;
window.skipDirectLinkAd = skipDirectLinkAd;
window.completeDirectLinkAd = completeDirectLinkAd;
